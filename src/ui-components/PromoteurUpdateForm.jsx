...
import { getPromoteur } from "../graphql/queries";
import { updatePromoteur } from "../graphql/mutations";
const client = generateClient();
export default function PromoteurUpdateForm(props) {
  const {
    id: idProp,
    promoteur: promoteurModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = promoteurRecord
      ? { ...initialValues, ...promoteurRecord }
      : initialValues;
  ...
  const [promoteurRecord, setPromoteurRecord] =
    React.useState(promoteurModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getPromoteur.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getPromoteur
        : promoteurModelProp;
      setPromoteurRecord(record);
    };
    queryData();
  }, [idProp, promoteurModelProp]);
  React.useEffect(resetStateValues, [promoteurRecord]);
  const validations = {
    nom: [{ type: "Required" }],
    email: [{ type: "Required" }],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updatePromoteur.replaceAll("__typename", ""),
            variables: {
              input: {
                id: promoteurRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "PromoteurUpdateForm")}
      {...rest}
    >
      <TextField
        label="Nom"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || promoteurModelProp)}
